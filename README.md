...
| kNN | Has Accuracy 0.956, MCC 0.906, very close to Logistic Regression. We have used scling on the data set to ensure Benefits from feature scaling (StandardScaler), since it relies on distance calculations between points. |
...
| **Overall ** | **Random Forest** — highest accuracy, highest MCC, highest precision, and tied-best recall among the top models. Its perfect precision (no false positives) combined with strong recall (0.929, missing only 3 of 42 malignant test cases) makes it the most reliable model on this dataset. |
