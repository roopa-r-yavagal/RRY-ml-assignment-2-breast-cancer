...
| Naive Bayes | Lowest recall (0.857) — missed 6 malignant cases (6 false negatives), this is a genuine concern for a diagnostic use case. This data set has features that are closely related (e.g. radius, perimeter, area), the independence assumption (treating all 30 features as uncorrelated) is the most likely cause of this poor performance. Its AUC (0.989) is still high — so it ranks cases well in terms of relative risk, but its default 0.5 decision threshold is not the correct choice for this data |
